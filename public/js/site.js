...
//assign images to project icons
  ...
  $(".icon").eq(3).css('background-image','url(../img/icons/SS.gif)');
  $(".icon").eq(4).css('background-image','url(../img/icons/CA.gif)');
...
//assign images to slides
 ...
 $(".siteImage").eq(3).css('background-image','url(../img/screens/streetsimon.jpg)');
 $(".siteImage").eq(4).css('background-image','url(../img/screens/calc.jpg)');
...
$('.description').eq(3).html('Worked with a classmate to build a Simple Simon game influenced by Street Fighter 2. Uses JavaScript, jQuery, and CSS with custom animation and sound. Best experienced with Chrome. <br>Buttons: \'R\': Red, \'F\': Green, \'U\': Blue, \'J\': Yellow');

$('.description').eq(4).html('Created using JavaScript, custom CSS, and HTML. Allows for negative and decimal numbers, and accounts for order of operation.');
